...
import { getCoins } from "../lib/progress";
...
export default function Header() {
  ...
  const [coins, setCoins] = useState<number>(() => getCoins(user?.id));
  ...
  // Listen for streak and coin updates
  useEffect(() => {
    const handler = () => fetchStreaks();
    const coinsHandler = () => setCoins(getCoins(user?.id));
    window.addEventListener("mathmaster:streaks-updated", handler);
    window.addEventListener("mathmaster:coins-updated", coinsHandler);
    window.addEventListener("mathmaster:progress-updated", coinsHandler);
    return () => {
      window.removeEventListener("mathmaster:streaks-updated", handler);
      window.removeEventListener("mathmaster:coins-updated", coinsHandler);
      window.removeEventListener("mathmaster:progress-updated", coinsHandler);
    };
  ...
  return (
    <header className="sticky top-0 z-50 bg-card border-b border-border shadow-sm">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 h-14 flex items-center justify-between gap-4">
        ...
        <Link
          to="/learn"
          className="flex items-center font-heading text-xl font-bold text-foreground hover:opacity-80 transition-opacity shrink-0 select-none"
          ...
          <span style={{ position: "relative", display: "inline-flex", alignItems: "baseline" }}>
            ...
            <span
              ...
              style={{
                ...
                backgroundClip: "text",
                filter: "drop-shadow(0 0 8px #818cf8cc) drop-shadow(0 0 16px #a855f766)",
                fontWeight: 900,
                lineHeight: 1,
              }}
            >
        ...
        <div className="flex items-center gap-3 shrink-0">
          ...
          <Link
            to="/shop"
            className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-amber-500/10 text-amber-600 dark:text-amber-400 border border-amber-500/20 text-xs font-heading font-bold shadow-sm hover:scale-105 hover:bg-amber-500/20 transition-all cursor-pointer"
            title="Card Shop & Balance"
          >
