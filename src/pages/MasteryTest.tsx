...
type Phase = "intro" | "loading" | "quiz" | "feedback" | "results" | "submitting";
...
export default function MasteryTest() {
  ...
  const startTest = async () => {
    ...
    for (const [topicSlug, metas] of byTopic.entries()) {
      const chosen = pickRandom(metas, QUESTIONS_PER_TOPIC, seed + qi * 999);
      for (const meta of chosen) {
        let qs = generateQuestions(meta.slug, meta.depth, 1, seed + qi * 137);
        
        // If mock, generate via Groq
        if (qs.length > 0 && qs[0].isMock) {
          try {
            ...
            qs = groqQs.map((q: any) => ({
              ...
              difficulty: meta.depth === "core" ? "easy" : meta.depth === "intermediate" ? "medium" : "hard",
              topic: meta.topicName,
              subtopic: meta.slug,
            }));
          } catch (e) {
            console.error("Mastery test AI generation error:", e);
            qs = []; // Skip question if generation fails
          }
        }
  ...
  if (phase === "loading") {
    return (
      <div className="flex flex-col items-center justify-center py-24 gap-4">
        ...
        <p className="text-foreground/60">Generating your mastery test…</p>
      </div>
    );
  }
