...
import { generateQuizQuestions, explainMistakes } from "../lib/groq";
import { setSubtopicStatus, setTopicFullyMastered, recordQuizAttempt } from "../lib/progress";
import type {
  QuizPhase,
  QuizResult,
  QuizQuestion,
...
import CalculatorBubble from "../components/CalculatorBubble";
...
export default function QuizPage() {
  ...
  useEffect(() => {
    const fetchKey = `${subtopicSlug}-${difficulty}-${count}-${seed}`;
    if (fetchRef.current === fetchKey) return;
    ...
    const run = async () => {
      ...
      if (qs.length === 0) {
        try {
          ...
          if (isMounted) {
            setPhase("error");
            fetchRef.current = null; // allow retry
          }
          return;
        }
      }
    ...
    return () => {
      // In React StrictMode, this cleanup runs immediately before the second effect.
      // If we set isMounted to false, the first fetch will drop its result.
      // But we PREVENT the second fetch using fetchRef.
      // So we MUST NOT drop the result of the first fetch!
      // Thus, we don't set isMounted = false here for the initial fetch.
    };
  ...
  // ─── Submit to DB & Progress ─────────────────────────────
  useEffect(() => {
    if (phase !== "submitting" || !result) return;
    (async () => {
      // Record quiz attempt to calculate coins
      recordQuizAttempt({
        subtopicSlug,
        difficulty,
        score: result.score,
      ...
      // Update subtopic & topic mastery
      if (result.passed) {
        if (subtopicSlug === "final-mastery") {
          setTopicFullyMastered(topicSlug, true, user?.id);
        } else if (difficulty === "mastery") {
          setSubtopicStatus(subtopicSlug, "mastered", user?.id);
        } else {
          setSubtopicStatus(subtopicSlug, "in_progress", user?.id);
        }
      }
      ...
      if (user) {
        ...
        if (error) {
          setSubmitError("We couldn't save your online result. Local progress was saved!");
        }
      }
      
      // Dispatch streak and progress update events
      window.dispatchEvent(new CustomEvent("mathmaster:streaks-updated"));
      window.dispatchEvent(new CustomEvent("mathmaster:progress-updated"));
      setPhase("coach");
    })();
  }, [phase, user, result, topicSlug, subtopicSlug, difficulty, passPercentage, seed]);
